...
const EditAutomationTaskPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const { tasks, updateTask } = useAutomationTasks();
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    if (task) {
      setForm({
        ...task,
        createdBy: task.createdBy || user?.displayName || user?.email || 'Current User',
      });
      if (task.type && !typeOptions.some(opt => opt.value === task.type)) {
        setForm(f => ({ ...f, type: 'custom' }));
        setCustomType(task.type);
      }
      if (task.schedule && !scheduleOptions.some(opt => opt.value === task.schedule)) {
        setForm(f => ({ ...f, schedule: 'custom' }));
        setCustomSchedule(task.schedule);
      }
    }
  }, [id, tasks, user]);
  ...
  return (
    <PageWrapper title="Edit Automation Task">
      <button
      ...
      <form onSubmit={handleSubmit} className="bg-white/90 backdrop-blur-sm rounded-lg shadow-lg border border-gray-200/20 p-8 max-w-6xl mx-auto">
        ...
        <div className="flex justify-end gap-4 mt-8">
          ...
          <button type="submit" className="px-6 py-2 rounded bg-blue-600 text-white hover:bg-blue-700" disabled={saving}>{saving ? 'Saving...' : 'Update'}</button>
        </div>
      </form>
    </PageWrapper>
  );
...
export default EditAutomationTaskPage; 